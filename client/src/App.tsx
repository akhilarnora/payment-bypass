...
const API_BASE_URL = 'https://sattvikonline.com/payment-checker/api/payment';
const PAYMENT_BASE_URL = "https://secure.payincharge.com/checkout/"
export const App: React.FC = () => {
  ...
  const viewMode = location.pathname === '/generate' ? 'GENERATOR' : 'CHECKOUT';
  ...
  return (
    <div className="min-h-screen w-full flex flex-col justify-between p-4 sm:p-6 lg:p-8">
      ...
      {/* <header className="w-full flex items-center justify-between py-4 max-w-5xl mx-auto border-b border-slate-900 mb-8 sm:mb-12">
        ...
        <div className="flex items-center space-x-4">
          {state === 'FORM' && (
            <button
              onClick={() => {
                const nextPath = viewMode === 'CHECKOUT' ? '/generate' : '/';
                navigate(nextPath);
              }}
            ...
            >
              {viewMode === 'CHECKOUT' ? 'Link Generator' : 'Back to Home'}
            </button>
          )}
      ...
      {/* <main className="flex-1 flex items-center justify-center w-full max-w-5xl mx-auto mb-12 sm:mb-20">
        ...
        {viewMode === 'CHECKOUT' && (state === 'FORM' || state === 'LOADING') && (
        ...
        {viewMode === 'GENERATOR' && (state === 'FORM' || state === 'LOADING') && (
          <LinkGenerator
            isLoading={state === 'LOADING'}
            setIsLoading={(ld) => setState(ld ? 'LOADING' : 'FORM')}
          />
        )}
